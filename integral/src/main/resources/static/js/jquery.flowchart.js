...
//			var scrollTop = $("#userMergeTableColumnsDivIdDXP").scrollTop();
			var scrollTop = this.element[0].scrollTop;
			 y = (connectorOffset.top - elementOffset.top - 1 + scrollTop) / this.positionRatio + parseInt($connector.css('border-left-width'), 10);
        ...
        _refreshLinkPositions: function (linkId) {
            ...
            var parentDivId = $(this.element).attr('id');
            ...
            // Calculate the ending point for the line
            bezierToX = toX + 1;

            // Adjust the Y coordinates if necessary
            toY = toY + 5;
            fromY = fromY + 5;

            // Set the path attribute to draw a straight line from the starting point to the ending point
            linkData.internal.els.path.setAttribute("d", 'M' + bezierFromX + ',' + fromY + ' L' + bezierToX + ',' + toY);

            // Adjust the rectangle position if needed (assuming this is part of your logic)
...
//                linkData.internal.els.path.setAttribute("d", 'M' + bezierFromX + ',' + (fromY) + ' L' + xdistanceCenter + ',' + fromY + ' ' + xdistanceCenter + ',' + toY + ' ' + bezierToX + ',' + toY);
        ...
        _connectorClicked: function (operator, connector, subConnector, connectorCategory) {
            if (connectorCategory == 'outputs') {
				...
				this.isFromConnectorClicked = true;
                this._mousemove(x, y);
            }
		...
		if (!this.isFromConnectorClicked) {
			  var scrollY = this.element[0].scrollTop;
			  y += scrollY;
			  }
			  console.log(x+" "+y);
			...
			this.isFromConnectorClicked = false;
        },
